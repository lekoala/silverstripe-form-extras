 ...
 */
class TimeDropdownField extends DropdownField
{
    ...
    public function __construct($name, $title = null, $value = "")
    {
        ...
        $source = $this->hoursRange(0, 86400, 3600 / 4, 'H:i');
    ...
    public function hoursRange($lower = 0, $upper = 86400, $step = 3600, $format = '')
    {
        ...
        foreach (range($lower, $upper, $step) as $increment) {
            ...
            $times[(string) $increment . ':00'] = $date->format($format);
        }
    ...
    public function validate($validator)
    {
        ...
        if (!Zend_Date::isDate($this->value, $this->getConfig('timeformat'), $this->locale)) {
            $validator->validationError(
                $this->name, _t(
                    'TimeField.VALIDATEFORMAT', "Please enter a valid time format ({format})", array('format' => $this->getConfig('timeformat'))
                ), "validation", false
            );
            return false;
        }
